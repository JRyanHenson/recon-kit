...
def parse_burp_scope(path):
    ...
    for entry in entries:
        ...
        is_wildcard = False
        if DEBUG:
            print(f"  [DEBUG] host_pattern: {repr(host_pattern)}")
            print(f"  [DEBUG] startswith ^.+\\\\.: {host_pattern.startswith('^.+\\.')}")
            print(f"  [DEBUG] startswith ^.*\\\\.: {host_pattern.startswith('^.*\\.')}")
        ...
        # If wildcard, mark subdomain scanning on the base domain too
    ...
    # Second pass: link wildcards to base domains that were added later
